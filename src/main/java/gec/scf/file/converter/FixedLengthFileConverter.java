...
public class FixedLengthFileConverter<T> extends AbstractFileConverter<T> {
		...
		try {
			...
			while ((currentLine = bufferReader.readLine()) != null) {
				...
				if (!hasCheckedFooter) {
					...
					if (StringUtils.isBlank(recordType.trim())) {
						// Found a blank recoredId
						FileLayoutConfigItem footerRecordType = footerRecordTypeExtractor.getConfig();
						throw new WrongFormatFileException(
				...
				if (fileLayoutConfig.getDetailFlag().equals(recordType)) {
					...
					if (detailDocAmountLayoutConfig != null) {
						try {
							...
							String data = getCuttedData(detailDocAmountLayoutConfig, currentLine);
							...
							if (detailDocAmountLayoutConfig.getSignFlagConfig() != null) {
								...
								String signFlagData = getCuttedData(detailDocAmountLayoutConfig.getSignFlagConfig(),
										currentLine);
								totalDetailAmount = applySignFlag(totalDetailAmount, detailDocAmountLayoutConfig,
										signFlagData);
	...
	private void validateFooter(String currentLine, List<FileLayoutConfigItem> footerConfigItems)
				...
				if (totalDetailRecord != footeTotalDocuments) {
					throw new WrongFormatFileException(
							MessageFormat.format(CovertErrorConstant.FOOTER_TOTAL_LINE_INVALIDE_LENGTH_MESSAGE,
									footerTotalDocLayoutConfig.getDisplayValue(), footeTotalDocuments,
									totalDetailRecord),
							currentLineNo);
				}
			}
		...
		if (footerTotalDocAmountLayoutConfig != null) {
			...
			String totalDocAmoutData = getCuttedData(footerTotalDocAmountLayoutConfig, currentLine);
			try {
				...
				if (footerTotalDocAmountLayoutConfig.getSignFlagConfig() != null) {
					...
					String signFlagData = getCuttedData(footerTotalDocAmountLayoutConfig.getSignFlagConfig(),
							currentLine);

					footerTotalAmount = applySignFlag(footerTotalAmount, footerTotalDocAmountLayoutConfig,
							signFlagData);
				...
				if (footerTotalAmount.compareTo(totalDetailAmount) != 0) {
					throw new WrongFormatFileException(
							MessageFormat.format(CovertErrorConstant.FOOTER_TOTAL_AMOUNT_INVALIDE_LENGTH_MESSAGE,
									footerTotalDocAmountLayoutConfig.getDisplayValue(), footerTotalAmount.doubleValue(),
	...
	private void validateExpectedValue(FileLayoutConfigItem item, String dataValidate) throws WrongFormatFileException {
		...
		if (StringUtils.isBlank(dataValidate)) {
			String errorMessage = MessageFormat.format(CovertErrorConstant.ERROR_MESSAGE_IS_REQUIRE,
		...
		for (FileLayoutConfigItem fileLayoutConfig : list) {
			if ("recordId".equals(fileLayoutConfig.getDocFieldName())) {
				...
				case HEADER:
					...
					break;
				case FOOTER:
					if ("totalDocumentAmount".equals(fileLayoutConfig.getDocFieldName())) {
						footerTotalDocAmountLayoutConfig = fileLayoutConfig;
					}
					if ("totalDocumentNumber".equals(fileLayoutConfig.getDocFieldName())) {
	...
	private T convertDetail(String currentLine) {
		...
		for (FileLayoutConfigItem config : fileLayoutConfigs) {
			if (StringUtils.isNotBlank(config.getDocFieldName()) && config.getDocFieldName().equals("recordId")) {
				continue;
			}
			...
			try {
				...
				if (StringUtils.isNotBlank(config.getDocFieldName())) {
					...
					if (config.getSignFlagConfig() != null) {
						...
						int endSignFlag = (signFlagConfig.getStartIndex() + signFlagConfig.getLenght()) - 1;
